...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
			...
			// (2-3)寻找所有与currentJNode邻接且未曾被访问的节点，将它们插入openList中
	...
	private void estimateValue(JigsawNode jNode) {
		...
		for(int index =1 ; index <= dimension*dimension; index++){
			if(index < dimension*dimension && 
			   jNode.getNodesState()[index]+1!=jNode.getNodesState()[index+1])
				nextWrongNum++;
			
			if(jNode.getNodesState()[index]!=this.endJNode.getNodesState()[index])
				wrongNum++;

			if (jNode.getNodesState()[index] != 0 && 
				jNode.getNodesState()[index] != this.endJNode.getNodesState()[index] )
			{	
				int x = ( index - 1 + dimension) % dimension; // ( 1 - 5 column 0 - 4)
				int y = ( index - 1 ) / dimension; // ( 1 - 5 row  0 )
				
				for (int i =1 ; i <= dimension*dimension; i++){
					if (jNode.getNodesState()[index] == this.endJNode.getNodesState()[i])
					{
						int dx = ( i - 1 + dimension) % dimension; // ( 1 - 5 column 0 - 4)
						int dy = ( i - 1 ) / dimension; // ( 1 - 5 row  0 )
						
						int disX = Math.abs(x - dx); 
						int disY = Math.abs(y - dy );
						
						manhattenDistance += disX + disY;
						actualDistance += (int) Math.sqrt(disX * disX + disY * disY);
						break;
					}
		...
		jNode.setEstimatedValue(2 * manhattenDistance + 1 * actualDistance + nextWrongNum + 0 * wrongNum);
	}
